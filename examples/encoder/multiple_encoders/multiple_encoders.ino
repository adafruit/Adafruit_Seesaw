/* Demo with 128x64 OLED display and multiple I2C encoders wired up. The sketch
 * will auto- detect up to 4 encoder on the first 4 addresses. Twisting will
 * display text on OLED and change neopixel color. set USE_OLED to true t
 */
...
void loop() {
  ...
  for (uint8_t enc = 0; enc < sizeof(found_encoders); enc++) {
    ...
    if (encoder_positions[enc] != new_position) {
      ...
      // change the neopixel color, mulitply the new positiion by 4 to speed it
      // up
